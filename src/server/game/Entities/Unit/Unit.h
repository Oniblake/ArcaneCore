...
class TC_GAME_API Unit : public WorldObject
{
    ...
    public:
        float chatbox_modifier = 1.0;

        uint32 stat_pvp_free = 0;
        uint32 stat_pvp_damage = 1;
        uint32 stat_pvp_armor = 1;
        uint32 stat_pvp_critical = 1;
        uint32 stat_pvp_heal = 1;

        uint32 stat_health_min = 5;
        uint32 stat_health_max = 11;

        uint32 stat_pve_free = 0;
        uint32 stat_pve_attack_melee = 1;
        uint32 stat_pve_attack_ranged = 1;
        uint32 stat_pve_attack_magic = 1;

        uint32 stat_pve_defense_melee = 1;
        uint32 stat_pve_defense_ranged = 1;
        uint32 stat_pve_defense_magic = 1;
        uint32 stat_pve_heal = 1;

        uint32 ochko_deistviya = 0;

        bool isAddonEnabled = false;
};
